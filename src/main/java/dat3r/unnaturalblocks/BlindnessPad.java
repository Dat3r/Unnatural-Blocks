...
public class BlindnessPad extends FacingBlock {
    public BlindnessPad(Settings settings) {
        super(settings);
    }
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        if (!world.isClient()) { // if it's on the server side
            if (entity instanceof LivingEntity) { // check if this entity is a living entity (as opposed to a armour stand or arrow or something)
                ...
                if (!le.hasStatusEffect(StatusEffects.BLINDNESS)) { // if it doesn't already have speed
                    world.playSound(null, pos.getX(), pos.getY(), pos.getZ(), SoundEvents.ENTITY_DROWNED_AMBIENT, SoundCategory.BLOCKS, 1.0f, 1.0f); // play a sound. pling!
                }

                StatusEffectInstance SpeedpadStatusEffectInstance = new StatusEffectInstance(StatusEffects.BLINDNESS, 80, 10, true, false); // create a status effect instance of speed
